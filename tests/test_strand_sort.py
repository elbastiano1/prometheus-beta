import pytest
from src.strand_sort import strand_sort

def test_strand_sort_basic():
    """Test basic sorting of a mixed list of integers."""
    input_list = [5, 2, 9, 1, 7, 6, 3]
    expected = sorted(input_list)
    assert strand_sort(input_list) == expected

def test_strand_sort_already_sorted():
    """Test sorting a list that is already sorted."""
    input_list = [1, 2, 3, 4, 5]
    assert strand_sort(input_list) == input_list

def test_strand_sort_reverse_sorted():
    """Test sorting a list in reverse order."""
    input_list = [5, 4, 3, 2, 1]
    expected = sorted(input_list)
    assert strand_sort(input_list) == expected

def test_strand_sort_empty_list():
    """Test sorting an empty list."""
    assert strand_sort([]) == []

def test_strand_sort_single_element():
    """Test sorting a list with a single element."""
    input_list = [42]
    assert strand_sort(input_list) == input_list

def test_strand_sort_with_duplicates():
    """Test sorting a list with duplicate elements."""
    input_list = [3, 1, 4, 1, 5, 9, 2, 6, 5, 3]
    expected = sorted(input_list)
    assert strand_sort(input_list) == expected

def test_strand_sort_with_floats():
    """Test sorting a list of floating-point numbers."""
    input_list = [3.14, 2.71, 1.41, 0.58, 2.23]
    expected = sorted(input_list)
    assert strand_sort(input_list) == expected

def test_strand_sort_with_strings():
    """Test sorting a list of strings."""
    input_list = ['banana', 'apple', 'cherry', 'date']
    expected = sorted(input_list)
    assert strand_sort(input_list) == expected

def test_strand_sort_type_error():
    """Test that a TypeError is raised for non-list inputs."""
    with pytest.raises(TypeError):
        strand_sort("not a list")
    
    with pytest.raises(TypeError):
        strand_sort(123)

def test_strand_sort_non_comparable():
    """Test handling of non-comparable elements."""
    class NonComparable:
        pass
    
    with pytest.raises(TypeError):
        strand_sort([NonComparable(), NonComparable()])